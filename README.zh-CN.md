...
**双机验收已完成。** 两台机器之间的真实会话——在一台 Mac 上经 P2P 通道驱动 Windows 主机上的 DSH Web 运行时（使用线上协调服务器）——已完成端到端验证：连接报告了选中的路径，打开了本地网关 URL，并经隧道加载了远端 DSH Web 页面。
...
7. **运行**：使用本地页签，并按需打开远程电脑页签来访问 DSH Web 会话。
