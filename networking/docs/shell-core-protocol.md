...
``managed.checkout_prepare`, `managed.checkout_verify`, `managed.git_register`and`managed.repository_inspect`are the checkout layer the core took over in 4.2. The
...
—`managed.git_remote_invalid`, `managed.git_revision_invalid`and`managed.git_operation_failed`— because the shell's own`git.*` codes are
