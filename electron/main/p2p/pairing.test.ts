...
describe('main-owned P2P pairing', () => {
  ...
  it('skips a listed pair that names neither side of this device', async () => {
    // A device identity this machine no longer holds leaves entries behind that
    // belong to two other devices. Failing the read for one of them aborted the
    // whole member sync — no pins, no catalog — so the machine kept seeing only
    ...
    f.call.mockImplementation(async (method: string) => {
      if (method === 'pairs.list') return [foreign, mine]
      if (method === 'pairs.identity') return identity('active')
      throw new Error(`unexpected ${method}`)
    })
    const members = await f.pairing.members(serviceId, signal())
    expect(members.map((member) => member.target.deviceId)).toEqual([remoteDevice])
  })
  ...
  it('skips one pair whose identity names neither side while reading the rest', async () => {
    ...
    f.call.mockImplementation(async (method: string, payload: unknown) => {
      ...
      if (method === 'pairs.identity') {
        const data = (payload as { data: { pairId: string } }).data
        return data.pairId === foreignPair ? foreign : identity('active')
      }
      throw new Error(`unexpected ${method}`)
    })
    const members = await f.pairing.members(serviceId, signal())
    expect(members.map((member) => member.pairId)).toEqual([pairId])
  })
})
