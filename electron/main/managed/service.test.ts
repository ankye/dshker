...
/**
 * Stands in for the core's installation catalog: the same file, the same bytes
 * and the same refusals, without an RPC. The real owner is covered by the Go
 * package and by integration/install_catalog_test.go; the catalog is written
 * here once during setup and only ever read afterwards.
 */
function fixtureInstallCatalog(): CoreInstallCatalogPort {
  return {
    async inspect(location: CoreInstallCatalogLocation): Promise<ManagedInstallationCatalog> {
      const text = await readFile(location.filePath, 'utf8').catch(() => undefined)
      if (text === undefined) {
    ...
    async commit(
      ...
      await mkdir(nodePath.dirname(location.filePath), { recursive: true })
      await writeFile(location.filePath, `${JSON.stringify(catalog, null, 2)}\n`, 'utf8')
      return catalog
    }
  }
}
...
async function fixture(): Promise<{
  ...
  const coreInstallCatalog = fixtureInstallCatalog()
  const service = new ManagedWorkspaceService({
    roots: () => coreRoots,
    installCatalog: () => coreInstallCatalog,
