...
| `p2p.signalling_not_ready`                                                                   | This computer's own signalling subscription is still being rebuilt, usually right after a previous connection to the same peer ended. It retries on its own; the server is not at fault. |
...
| `p2p.stream_failed` / workbench shows a 502 page               | The connection came up but requests could not reach the remote workbench — almost always a firewall dropping UDP, or the two machines being unreachable from each other. Check both firewalls allow UDP (macOS: System Settings → Network → Firewall; Windows: Windows Security → Firewall & network protection) and that both computers are on the same reachable network (same Wi-Fi / VPN / LAN). If it drops right after connecting, the keep-alive packets are being lost; configure the server's TURN relay. |
