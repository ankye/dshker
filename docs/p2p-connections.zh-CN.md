...
- 任意网络都能连通的方案。直连需要可用的 UDP 路径。某些网络无法穿透——两端都是
  对称 NAT、UDP 被完全封锁、企业出口严格限制。直连无法建立时，连接会回退到
  **你自己部署的服务器作为不透明中继**（TURN，RFC 8656）：服务器只转发端到端加密的
  报文流（DTLS/SCTP 密文），无法读取或注入任何业务流量。若直连与中继路径都
  无法建立，连接如实报 `direct_unavailable` 并失败。
...
首次测试最常卡在两件事上：本机没有可用的 core 二进制，或者协调服务器实际不可达。
配对之前先检查这两项：
...
- **core 缺失或校验和不符。** 每台机器必须为自己的架构各自构建 core 二进制，不能从
  别的机器拷贝。失败信息里会直接给出该跑的命令。
- **TLS 证书不受信。** 请在服务器侧解决。应用会主动拒绝无法验证的服务器，因此本机
  不信任的自签名证书是连不上的。
- **STUN 无响应。** UDP 被封锁或端口未开放。这正是"配对成功但连接报
  `direct_unavailable`"最常见的原因。中继回退同样走 UDP（到你的服务器），
  因此 UDP 完全不通的网络仍然无法穿透，必须修好而不能绕过。
...
2. 在 **Run** 下，点击页签栏“+”，从浮层的“局域网电脑”列表选择这台电脑按需创建标签页；未建立连接的设备会保留在列表底部并置灰。
...
- **电脑名称**仅作用于本机。随时改名都是安全的，不会中断已有连接，也不会重载已打开的标签页。
...
一个网络的设备列表**只有一份，只有一个维护者**：由 core 维护。它会在本机登录时、在任何改变网络成员的操作之后（登记、绑定、退出、新建/删除网络、配对），以及登录期间每 30 秒去服务器读一次——30 秒已经是最细的有意义间隔，因为服务器对一台电脑的 last_seen 每分钟最多落盘一次。内容真的变了，core 会通知窗口，所有显示这份列表的界面自己就更新了。

所以一台电脑上线或离线，不需要你手动刷新任何页面；同一个网络里的两台电脑也不会再各自显示不同的成员——它们各自从自己的 core 读同一种快照，而不是每个页面自己去读服务器、各留一份副本。列表旁边的 **刷新列表** 是让 core 立刻去服务器读一次，只有你不想等下一次读取时才需要。

### 已配对的电脑列表

运行页里可连接的那份电脑列表出于同样的理由由同一个人维护：core。它在同一个 30 秒节奏里读取服务器的配对关系，并且在任何可能改变配对的操作之后立刻再读一次（加入或退出网络、绑定或解绑设备、批准、撤销、认领配对），为每个生效的配对写入 pin，再把结果记下来。内容变化时 core 通知窗口，所以在别处刚配好的电脑会**在列表打开着的时候就出现**，不用等下次打开列表。

也正因为如此，打开运行页不再向服务器请求任何东西：它读的是 core 已经持有的数据；而一个从不打开的页面，也不会让这份列表过期。

| 错误码                     | 含义与处理                                                                                                 |
| -------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `p2p.user_login_required`  | 这台服务器上没有登录状态，因此没有可维护、可显示的列表。登录即可；列表跟随本机当前登录的账号。             |
| 某一行一直显示「从未上报」 | 电脑已登记并已绑定，但登记之后从未上报过。它会一直显示为离线，直到那台电脑重新运行并登录。                 |
| 某一行完全不见了           | 可能是网络所有者把它移除了，也可能是那台电脑登录的是另一个账号。点 **刷新列表** 重新读一次；以服务器为准。 |
...
已连接对方的工作台，是本机上的一个 loopback 网关：core 打开一个监听端口，经加密的
点对点会话转发到对方的 DSH Web，而打开它的 URL 查询串里带着 token。这个 URL 会到达
渲染进程，因为对方工作台显示在 `<webview>` 里，而 Electron 只能挂真实 `http` URL——
在主进程里自建 scheme 无法承载页面自身的 WebSocket 升级，而 DSH Web 需要它。

因此，把一台设备的会话与另一台隔开的是 **guest partition**，而不是地址的保密性：主进程
按配对的服务身份与 pair 身份推导分区（`peerPartition`），guest 只在那个分区里挂载，
另一台设备的 cookie 与 token 不会落在同一个会话里。本机 tab 与 SSH 远程同样是把带
凭据的 URL 交给同一个渲染进程。

地址也不会进入 tab 状态：它由命名的 `entry` 操作按某一次 attempt（generation）单独取回，
attempt 已被替换时直接拒绝，因此已经翻篇的 tab 不会拿到一个它的会话已不再拥有的网关。
把 guest 本身搬进主进程（`WebContentsView`）能让 token 完全不进渲染进程；这里记为后续
事项而没有一并做，因为它会改变 Run 页每一个 guest 的挂载、缩放与测量方式。
