...
- `add-managed-remote-dsh-connections` — `managed-remote-dsh-connections`, whose requirement now names the trusted core as the owner of OpenSSH and file-transfer resolution, subprocess arguments, temporary files, peer secrets, DSH session credentials, HTTP peer calls, persistence, and process shutdown, with the shell proxying named operations. The port itself (tasks 5.1 to 5.3) is what remains.
