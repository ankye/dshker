...
最新交付顺序（用户已确认）：完整功能与本地端到端/安全/打包门禁完成后，先发布 GitHub **prerelease**，明确用于 Win/Mac 联测，不标记 latest、不进入稳定更新源。物理跨平台与公网网络矩阵由该候选发布后开展；未执行项继续为 NOT_RUN/BLOCKED，不能借预发布宣称这些验证通过。以下正式 Release 要求继续约束稳定版晋级，不阻塞符合上述条件的联测预发布。
