...
远程工作台页签采用按需创建：启动时只挂载 Local；运行页签栏的“+”会展示“局域网电脑”和
“SSH 连接”两个可添加列表，用户选择后才创建并聚焦对应的 SSH/peer 页签。已创建页签在
连接中、已连接、断开、失败之间保留，只有权威电脑/配对记录被移除时才清理页签。20 台电脑
的启动验收必须断言初始只有 Local，逐台从“+”打开后才逐一出现页签，并且未打开的电脑
没有 WebView 实例。

添加页签面板必须是固定定位的浮层，不得参与 Run 浏览器布局或被浏览器内容撑开；在窄窗口
仍须限制宽度和可滚动高度，并从触发按钮恢复焦点。每个 LAN/SSH 列表按当前可建立工作台的
状态排序：ready 位于不可用项之前；connecting、disconnected、failed、revoked 等不可用项
保留在列表底部、显示真实状态并禁用选择，不得通过点击创建空白工作台。验收需覆盖浮层
边界、列表过长、空列表、禁用项和 Escape/外部点击关闭。
...
- [x] 11.4 直连受网络承载约束，已按用户决定转为中继回退：本现场跨机 UDP（ZeroTier 虚拟网）双向不通（ICMP/TCP 通、裸 UDP 丢弃），ICE 直连协商以 \`p2p.direct_unavailable\` 如实失败。用户决定：加服务器中转。实现为部署服务器上的不透明 TURN 中继（RFC 8656，只转发 DTLS/SCTP 密文、无法读取或注入），客户端 ICE 配置 TURN 凭据后直连失败自动回退中继；两种路径都失败才报 \`p2p.direct_unavailable\`。
- [x] 11.5 real-DSH 目标侧运行时：Windows 上由 dshker-server 反代/p2p 隧道驱动 DSH web 时，web 需以托管版本或自检出（versions 目录临时移开）运行；ssh 反代（TCP 承载）与 p2p 隧道承载的 HTTP 全链路均已验证：2026-09-12 live-drive 跨机实跑，mac 经 peersession/ICE 接通 win（A-CONNECTED/A-PATH），本端网关 URL 经隧道加载远端 DSH web（303+Cookie 认证后 A-PROBE 200 返回页面），同现场双端 go test 全绿。
...
- [x] 11.8 不透明中继（TURN）跨仓实现落地：dshker-server 以 pion/turn v5.1.1 在同一 UDP socket 兼答 STUN Binding 与 TURN 控制；`turnSharedSecret`（hex ≥32 字节）启用，TURN-REST 凭据（`<unix到期>:<设备ID>`、HMAC-SHA1、24h、不持久化）由 `/v1/turn-credentials` 按设备签发；dsh-launcher 的 controlplane 新增领取方法（管理器惰性缓存一次），`internal/peer` 的 ICE 服务器列表为 STUN 直连 + TURN 中继双条目，直连失败自动回退；凭据篡改（错误密码）在中继分配时被拒；未配置/过短明确 `p2p.relay_unconfigured`；旧配置兼容（核心字段仍严格、中继字段可选）。验证：服务器中继 4 项测试全过（格式/AuthHandler/双客户端 127.0.0.1 中继双向互通/篡改拒绝）、客户端新增单测全过、integration 全量绿（fixture 启用中继配置、旧二进制下凭据 404 优雅回落直连）。跨物理机实跑需等部署服务器换新二进制（见 11.4 边界）。
- [x] 11.9 跨机接通最后两处修复（2026-09-12，随 0.1.29 发布）：其一 `Transport.Path()` 拒绝任何含 relay 候选的选中对，中继接通的会话在首个 DataChannel 打开即被拆（改报 direct_unavailable）——现仅强制 UDP，relay 对为合法选中路径，ICE 仍优先直连；其二协商信号把租约到期时间填进自身 ExpiresAt，而对端校验窗口为 (now, now+60s]，每个 offer/answer 恒被拒为 `p2p.signal_expired`——新增 `protocol.SignalExpiry`（60s 有效期减 15s 时钟偏差余量）与 `TestSignalExpiry` 回归。干净二进制跨机复验：A-CONNECTED/A-PATH/A-GATEWAY/A-PROBE 200（DSH web 页面 + 会话 Cookie）；spec delta 同步改写为“直连优先 + 自部署不透明中继回退”（direct-peer-dsh-sessions）。
