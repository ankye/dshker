...
服务器读回里的 `pairId` 是 pairs 表行 id，**不能用它做连接相关查找的键**：连接请求、peer 会话的 pin 表、以及 core 维护的已配对电脑列表，一律以**对端 deviceId** 为键（发起方写目标 deviceId，接收方以发起方 deviceId 解析入站尝试）。三者一旦漂移，表现为每次连接都被拒绝为 `p2p.pair_unauthorized`，而自动重连把该码视为终态，于是配对在用户看来“还在但永远连不上”。回归测试断言这三个位置的 id 是同一个值。
