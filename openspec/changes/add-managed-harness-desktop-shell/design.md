...
Startup schedules one non-blocking check after the main window is usable. Only `update-available` creates a passive startup notice; a startup failure does not open a dialog or obscure the current page. Launcher Settings exposes the current state, last successful observation, explicit retry, and the exact version and asset name when an update is available. The renderer can request only the named check and download actions; it never supplies a repository, endpoint, URL, or destination. Electron main downloads the exact retained HTTPS asset to `app.getPath('downloads')`, publishes typed byte progress, and leaves manual installation to the user. Release bodies use explicit `### ... (zh-CN)` and `### ... (en-US)` sections; the renderer selects the active section and shows no notes when the selected section is absent.

Current installers are unsigned. The public Release and Settings copy identify in-app download and manual installation; no `electron-updater` path, background replacement, silent restart, or macOS auto-install is enabled. Signing and notarization must exist before a later change can consider native automatic installation.
...
## Launch home three-column layout (2026-09-07)

Below the retained hero, three peer cards show the selected Harness version, project/open-source information, and Launcher update status in that order. Wide windows use a 0.9:1.1:1 grid with existing spacing/surface tokens; windows at 980px or narrower stack the cards. The commit displays a 12-character summary, with its full value in a title and accessible label. Existing missing/preparing/retry states remain in the version column, while project and update information remain available independently. The shared update card supplies authoritative version/check/download state; the current protocol has no announcement body, so none is invented. Existing Settings title/description remain the defaults. The persistent launch footer stays outside the scroller. The approved otter workbench background replaces the old hero image, orbit and corner icon. The approved otter icon replaces the shared launcher PNG used by the sidebar, development window/Dock and electron-builder icon configuration. The hero artwork is fitted by height on the right without cropping the mascot; a dark overlay protects title contrast in compact windows.

Visual acceptance: three non-overlapping cards below the hero at 1240x820 and 1600x1000; single column at 760x560; wrapped long hashes and installer names; route footer always outside content scroll. Preserve keyboard actions and Chinese/English labels. No new animation, IPC or persistent data.

## Launch home visual refinement (2026-09-07)
